...
if (!is.gamlss(object))  stop(paste("This is not an gamlss object", "\n", ""))
 ...
 switch(nopar,  
    ...
    {pfun <- eval(call(dfun,q=object$y, mu=fitted(object,"mu"), sigma=fitted(object,"sigma"), nu=fitted(object,"nu")))},
    {pfun <- eval(call(dfun,q=object$y, mu=fitted(object,"mu"), sigma=fitted(object,"sigma"), nu=fitted(object,"nu"), tau=fitted(object,"tau")))})
pfun 
}
#----------------------------------------------------------
#=---------------------------------------------------------
...
if (!is.gamlss(obj))  stop(paste("This is not an gamlss object", "\n", ""))
 ...
 switch(nopar,  
    {lik <-if (fname%in%gamlss.bi.list)  eval(call(dfun,y=obj$y, bd=obj$bd,  mu=fitted(obj,"mu")))
             else   eval(call(dfun, y=obj$y, mu=fitted(obj)))},
    {lik <-if (fname%in%gamlss.bi.list)  eval(call(dfun,y=obj$y, bd=obj$bd,  mu=fitted(obj,"mu"), sigma=fitted(obj,"sigma") ))
...
if (modelPi)
   ...
   form.prob <- formula(paste("res.var~", pi.formula[[2]])) # get the formula
   ...
   { 
    ...
    DistPar <- fam$parameters
      nopar <- length(DistPar)  
       dfun <- paste("d",fname,sep="") 
   switch(nopar,  
      {lik <- if (fname%in%gamlss.bi.list) eval(call(dfun, y=y, bd=bd[[i]],  mu=mu[[i]]))
            else   eval(call(dfun, y=y, mu=mu[[i]]))
      },
      {lik <- if (fname%in%gamlss.bi.list) eval(call(dfun, y=y, bd=bd[[i]],  mu=mu[[i]], sigma=sigma[[i]]))
...
pMX <- function(q, 
 ...
 if(is.null(dim(q))) N <- length(q) else N <- dim(q)[1]  
   ...
   switch(nopar,  
     ...
     {lik <- eval(call(dfun,q=q, mu=mu[[i]], sigma=sigma[[i]], nu=nu[[i]]))},
     {lik <- eval(call(dfun,q=q, mu=mu[[i]], sigma=sigma[[i]], nu=nu[[i]], tau=tau[[i]]))})
    Prob[,i]<-pi[[i]]*lik
    } 
            fy <-rowSums(Prob)
       fy <- if(log == FALSE) fy else log(fy)
